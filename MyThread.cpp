...
bool MyThread::init(QString fileName,QString ip, int type, MyLogger *logger) {
    ...
    {
        qDebug() << "客户端winsock出错!";
        emit updateMsg("客户端winsock出错!");
        return false;
    }
    if (wsaData.wVersion != 0x0101)
    {
        qDebug() << "客户端winsock版本有误!";
        emit updateMsg("客户端winsock版本有误!");
        WSACleanup();
        return false;
    }
    //为随机数产生器分配种子
...
void MyThread::run() {
    if (type == THREAD_SEND) {
        ...
        receiveFile(fileName, ip);
    }
}

bool MyThread::startNet(QString serverIP) {
    // 初始化套接字
    serverAddr.sin_family = AF_INET;
    serverAddr.sin_port = htons(SERVER_PORT);
    serverAddr.sin_addr.s_addr = inet_addr(serverIP.toStdString().c_str());
    sock = socket(AF_INET, SOCK_DGRAM, 0);
    if(sock == INVALID_SOCKET){
        int error = WSAGetLastError();
        logger->log("socket() failed with error code : " + QString::number(error));
        emit updateMsg("socket() failed with error code : " + QString::number(error));
        return false;
    }
    ...
    if(sock == INVALID_SOCKET){
        ...
        emit updateMsg("bind() failed with error code : " + QString::number(error));
        return false;
    }
...
bool MyThread::stopNet() {
    closesocket(sock);
    logger->log("Stopped socket: "+QString::number(sock));
    return true;
}

void MyThread::uploadFile(const char* orifileName) {
    logger->log("Uploading file: "+QString(orifileName));
    emit updateMsg("Uploading file: "+QString(orifileName));

    //计算速度,每传输1000个包计算一次
    float aveSpeed = 0;   //平均速度 Mb/s
    float curSpeed = 0;   //当前速度 Mb/s
    long long lastBytesTransferred = 0;
    ...
    long long bytesTransferred = 0; //已传输的字节数

    //打开文件
    FILE *fp = fopen(orifileName, "rb");
    if(fp == nullptr){
        logger->log("Upload:File not found: "+QString(orifileName));
        emit updateMsg("Upload:File not found: "+QString(orifileName));
        return;
    }
    ...
    if(resendCount>=MAX_RESEND_COUNT){
        ...
        emit updateMsg("Upload:WRQ:Resend count exceeded maximum resend count");
        return;
    }
    ...
    do{
        ...
        for(resendCount = 0;resendCount < MAX_RESEND_COUNT;resendCount++){
            ...
            if(FD_ISSET(sock, &readfds)) {
                ...
                if(recvBuffer[1] == TFTP_ERROR){
                    ...
                    emit updateMsg("Download:ERROR:Error code: " + QString::number((unsigned char)recvBuffer[3]) + " Error message: " + QString::fromLocal8Bit(recvBuffer+4));
                    fclose(fp);
                    return;
                }
                WSAGetLastError();
        ...
        if(resendCount>=MAX_RESEND_COUNT){
            ...
            emit updateMsg("Upload:DATA:Timeout:Resend count exceeded maximum resend count");
            return;
        }
        ...
        if(!blockNum % 1000){
            //每传输1000个数据包,计算一次速度
            curSpeed = 1.0*(bytesTransferred - lastBytesTransferred) / (speedTimer.elapsed()*1000) ;
            aveSpeed = 1.0*bytesTransferred / (timer.elapsed()*1000) ;
            emit updateSpeed(aveSpeed, curSpeed);
    ...
    logger->log(retMsg);
    emit updateMsg(retMsg);
    return;
}

void MyThread::downloadFile(const char* fileName) {
    logger->log("Downloading file: "+QString(fileName));
    emit updateMsg("Downloading file: "+QString(fileName));
    ...
    if(fp == nullptr){
        ...
        emit updateMsg("Download:File:"+ QString::fromLocal8Bit(fileName) + " create failed");
        return;
    }
    ...
    if(sendto(sock, sendBuffer, len, 0, (sockaddr*)&serverAddr, sizeof(serverAddr)) == SOCKET_ERROR){
        ...
        emit updateMsg("Download:RRQ:sendto() failed with error code : " + QString::number(error));
        return;
    }
    else successCount++;
        ...
        for(resendCount = 0;resendCount < MAX_RESEND_COUNT;resendCount++){
            if(FD_ISSET(sock, &readfds)) {
                ...
                if(blockNum == 1){
                    ...
                    emit updateMsg("Download:DATA:Timeout:No data received");
                    //关闭文件
                    fclose(fp);
                    //删除文件
                    remove(fileName);
                ...
                if(resendCount == MAX_RESEND_COUNT){
                    ...
                    emit updateMsg("Download:DATA:Timeout:Resend count exceeded maximum resend count");
                    //关闭文件
                    fclose(fp);
                    //删除文件
                    remove(fileName);
    ...
    emit updateMsg(retMessage);
    return;
}

void MyThread::sendFile(QString fileName, QString serverIP) {
    //初始化
    if(!startNet(serverIP)){
        emit updateMsg("Send:Initialization failed");
        return;
    }
    //发送文件
    uploadFile(fileName.toStdString().c_str());
    //关闭连接
    stopNet();
}

void MyThread::receiveFile(QString fileName, QString serverIP) {
    //初始化
    if(!startNet(serverIP)){
        emit updateMsg("Receive:Initialization failed");
        return;
    }
    //接收文件
    downloadFile(fileName.toStdString().c_str());
    //关闭连接
    stopNet();
}
