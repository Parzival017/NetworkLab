...
void TFTPClient::start(const char * serverIP) {
    // 初始化套接字
    ...
    logger->log("Started socket: "+QString::number(sock)+" on port: "+QString::number(ntohs(clientAddr.sin_port)));

    //使用非阻塞模式
    u_long iMode = 1;
    int iResult = ioctlsocket(sock, FIONBIO, &iMode);
    if(iResult != NO_ERROR){
        qDebug() << "ioctlsocket() failed with error: " + QString::number(iResult);
    }
...
void TFTPClient::uploadFile(const char *orifileName) {
    ...
    for(resendCount = 0;resendCount < MAX_RESEND_COUNT;resendCount++){
        ...
        int time_wait_ack;
        for (time_wait_ack = 0; time_wait_ack < TIMEOUT; time_wait_ack += SLEEPTIME) {
            // 尝试接收
            ...
            if (recvSize == SOCKET_ERROR)//未接收到则等待20ms
                qDebug() << "wait";
                Sleep(SLEEPTIME);
        }
        if (time_wait_ack < TIMEOUT)//接收到则跳出循环
            break;
        else{
            //超时,重传WRQ包
            lossCount++;
        ...
        for(resendCount = 0;resendCount < MAX_RESEND_COUNT;resendCount++){
            ...
            for (time_wait_ack = 0; time_wait_ack < TIMEOUT; time_wait_ack += SLEEPTIME) {
                ...
                if (recvSize == SOCKET_ERROR) {
                    qDebug() << "time_wait_ack:" << time_wait_ack;
                    Sleep(SLEEPTIME);
                }
            }
            if(time_wait_ack < TIMEOUT)
                break;
            else{
                //超时,重传数据包
                lossCount++;
    ...
    logger->log("Upload:File:"+ QString::fromLocal8Bit(fileName) + " uploaded successfully, " + QString::number(bytesTransferred) + " bytes transferred");
    return;
}

void TFTPClient::downloadFile(const char *fileName) {

    long successCount = 0; //成功发送的数据块数
    long lossCount = 0; //丢失的数据块数
    long transBytes = 0; //传输的字节数
    ...
    do{
        ...
        if(time_wait_ack >= TIMEOUT && blockNum == 1){
            logger->log("Download:DATA:Timeout:No data received");
            break;
        }
        if(time_wait_ack >= TIMEOUT && blockNum > 1 && resendCount < MAX_RESEND_COUNT){
            //超时,重传ACK
            lossCount++;
        ...
        if(time_wait_ack >= TIMEOUT && resendCount == MAX_RESEND_COUNT){
            //重传次数超过最大重传次数
            logger->log("Download:DATA:Timeout:Resend count exceeded maximum resend count");
            fclose(fp);
            return;
        }
        transBytes += recvSize - 4;
        blockNum++;
    }while(recvSize == MAX_DATA + 4);	// 当数据块未装满时认为时最后一块数据，结束循环

    //关闭文件
    fclose(fp);
    logger->log("Download:File:"+ QString::fromLocal8Bit(fileName) + " downloaded successfully, " + QString::number(transBytes) + " bytes transferred");
}

void TFTPClient::sendFile(const char* fileName,const char * serverIP){
    //初始化
    start(serverIP);
    //发送文件
    uploadFile(fileName);
...
void TFTPClient::receiveFile(const char* fileName,const char * serverIP){
    //初始化
    start(serverIP);
    //接收文件
